...
![Screenshot 1]( "1. Установка сервера Ubuntu")

##Part 2. Создание пользователя
Создаем пользователя coolAdmin через команды, представленные на скриншотах
![Screenshot 1]( "1. Создание нового пользователя, установка пароля для него и добавление в группу adm")
Проверяем его нахождение в группе adm
![Screenshot 2]( "2. Проверка нахождения пользователя в группе adm")

##Part 3. Настройка сети ОС
Меняем имя хоста на user-1 через sudo vim /etc/hostname
![Screenshot 1]( "1. Имя хоста изменено на user-1")
Проверяем текущее локальное время timedatectl
![Screenshot 2]( "2. Локальное время до изменения часового пояса")
Устанавливаем локальное время по часововму поясу Москвы
![Screenshot 3]( "3. Изменение часового пояса на московский")
...
![Screenshot 4]( "4. Название сетевых интерфейсов, сброшен старый ip-адрес")
...
![Screenshot 5]( "5. Получение нового ip-адреса устройства от DHCP сервера")
Определяем внешний и внутренний ip адреса
![Screenshot 6]( "6. Внешний и внутренний ip-адреса")
Задаем статичные настройки ip, gw, dns
![Screenshot 7]( "7. Настройки  файла 00-installer-config.yaml для сети enp0s3")
Перезагружаем систему sudo reboot. Проверяем сетевые настройки - совпадают
![Screenshot 8]( "8. Новый статический ip-адрес")
Пропинговываем удаленные хосты 1.1.1.1 и ya.ru. Убеждаемся, что нет потерь пакетов
![Screenshot 9]( "9. Пингование удаленного хоста, 0% потерь пакетов")
![Screenshot 10]( "10. ping 1.1.1.1")
...
![Screenshot 1]( "1. Скачивание и установка обновлений через команды apt update / apt dist-upgrade")
...
![Screenshot 1]( "1. Изменение имени hostname через пользователя coolAdmin с правами root")

##Part 6. Установка и настройка службы времени
Установка network time protocol sudo apt install -y ntp
![Screenshot 1]( "1. Установка network time protocol")
![Screenshot 2]( "2. Список подключенных серверов времени")
Останавливаем ntp и синхронизируем время
![Screenshot 3]( "3. Остановка ntp и принудительная синхронизация времени")
Запуск ntp и проверка состояния службы
![Screenshot 4]( "4. Запуск ntp и проверка состояния службы")
Проверка синхронизации - не синхронизированно. После удаления ntp синхронизация состоялась
![Screenshot 5]( "5. Проверка синхронизации - no. Удаляем ntp, после чего синхронизация - yes")

##Part 7. Установка и использование текстовых редакторов
VIM и NANO уже предустановлены в Ubuntu, устанавливаем только MCEDIT
![Screenshot 3]( "3. Установка mcedit")
